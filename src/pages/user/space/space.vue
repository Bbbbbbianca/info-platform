<template>
  <view class="tf-space-container">
    <view class="tf-space-info">
      <view class="tf-space-avatar">
        <AtAvatar
          ...
          class="tf-space-avatar-size"
        />
      </view>

      <view class="tf-space-list">
        <view class="tf-space-list-item">
          <view class="tf-space-list-item-title">
            昵称
          </view>
        ...
        <view class="tf-space-list-item">
          ...
          <view
            v-if="user.break===0"
            class="tf-space-list-item-break-text1"
          >
            本月无不良记录
          </view>
          <view
            v-else
            class="tf-space-list-item-break-text2"
          >
            本月有{{ user.break }}次不良记录
          </view>
          <view
            class="tf-space-list-item-break-hint"
            @tap="onShowHint"
          >
            <AtIcon
              value="alert-circle"
              class="tf-space-list-item-break-hint-icon"
            />
          </view>
        </view>
      </view>
    ...
    <AtActionSheet
      cancel-text="取消"
      :is-opened="showHint"
      :on-cancel="onHideHint"
      :on-close="onHideHint"
    >
      <AtActionSheetItem>
        <view class="tf-space-action-hint">
          报名活动后不参加（取消报名除外）将会被记为违规
        </view>
        <view class="tf-space-action-hint">
          三次以上违规将在一个月内不能报名任何活动
        </view>
      </AtActionSheetItem>
      <AtActionSheetItem
        :on-click="onShowAppeal"
        style="color: #d32f2f;"
      >
        申诉
      </AtActionSheetItem>
    ...
    <view
      v-if="showAppeal"
      class="tf-space-dialog"
    >
      <view class="tf-space-dialog-card">
        <view class="tf-space-dialog-title">
          申诉理由
        </view>
        <view class="tf-space-dialog-context">
          <AtTextarea
            ...
            style="background-color: #f7f7f7; font-size: 14px; border: none;"
          />
        </view>
        <view class="tf-space-dialog-action">
          <view
            class="tf-space-dialog-cancel"
            @tap="cancelAppeal"
          >
            取消
          </view>
          <view
            class="tf-space-dialog-confirm"
            @tap="confirmAppeal"
          >
            提交申请
          </view>
